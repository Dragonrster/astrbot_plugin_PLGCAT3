...
class MyPlugin(Star):
    ...
    def _generate_fortune(qqid: str, date_str: str) -> dict:
        ...
        # 每组5项，按分类权重均分
        #  大吉8  中吉15  小吉18  吉18  末吉14  平10  平吉8  凶5  大凶1  小凶2  凶中带吉1
        weights = (
            [2] * 5   # 大吉
            + [3] * 5   # 中吉
            + [4, 4, 4, 3, 3]  # 小吉
    ...
    async def mcmanhelp(self, event: AstrMessageEvent):
        ...
        text = "\n".join(
            [
                ...
                "  [管] /mckill <游戏ID>  击杀",
                "  [管] /mcauthunregister <游戏ID>  AuthMe注销（注销）",
                "",
                "【服务器】",
                "  /mclist  在线玩家（mcl）",
                "  /mcplugins  插件列表",
                "  [管] /mctps  TPS",
                "  [管] /mcsparkhealth  Spark健康（mcspark/mcsh）",
                "  [管] /mcping [目标]  ping（ping/mcp）",
                "  [管] /mcentitylist [选择器] [世界]  实体列表（mcel）",
                "  /mcstats [@某人|游戏ID]  玩家统计（mcstat）",
                "  [管] /mcrun <命令>  RCON透传（mcexec）",
                "",
                "【聊天】",
                "  /mcsay <内容>  游戏内说话（mcs）",
                "  [管] /mcbroadcast <内容>  游戏内广播（mcb/mcbc）",
                "  [管] /mcbindchat  绑定聊天转发目标",
                "  [管] /撤回 <秒数>  开启自动撤回模式",
                "",
                "帮助：/mcmanhelp（mchelp/mch）",
            ]
        )
    ...
    async def mcwithdraw(self, event: AstrMessageEvent):
        ...
        try:
            ...
            yield event.plain_result("用法：.撤回 <秒数>  例如 .撤回 600")
            return
    ...
    async def _withdraw_interceptor(self, event: AstrMessageEvent):
        ...
        if not gid or gid not in self._withdraw_until:
            return
        if time.time() >= self._withdraw_until[gid]:
            del self._withdraw_until[gid]
            return
        ...
        try:
            ...
            logger.info(f"[撤回] 已撤回 msg={msg_id} sender={sender_id}")
        except Exception:
            pass
